...
function Dashboard({ user: initUser, onLogout }) {
  ...
  return (
    <div style={s.dash}>
      ...
      {tab === 'deposit' && (<div style={s.tc}><h3 style={{ color: '#28a745' }}>📥 إيداع USDT (TRC20)</h3>
        <div style={{ background: 'var(--bg2)', padding: 15, borderRadius: 10, margin: '15px 0', border: '1px dashed var(--accent)' }}>
          <p style={{ fontSize: 12, color: 'var(--text3)' }}>عنوان المحفظة (TRC20):</p>
          <code style={{ color: 'var(--accent)', wordBreak: 'break-all', fontSize: 12 }}>TLhmbZbsvRhf2TpGiotkHnbv7YBfxbKprn</code>
        </div>
        <div style={{ background: '#1a2a1a', padding: 12, borderRadius: 8, marginBottom: 12, border: '1px solid var(--accent)' }}>
          <p style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 'bold', marginBottom: 6 }}>📋 خطوات الإيداع:</p>
          <p style={{ fontSize: 12, marginBottom: 4 }}>1️⃣ روح لمكتب صرافة وقول له حوّل <strong>USDT على شبكة TRC20</strong> للعنوان أعلاه</p>
          <p style={{ fontSize: 12, marginBottom: 4 }}>2️⃣ بعد ما يحوّل، اطلب منه <strong>رقم المعاملة (TxID)</strong> — رقم طويل من Tronscan</p>
          <p style={{ fontSize: 12, marginBottom: 4 }}>3️⃣ صوّر <strong>إيصال أو شاشة التحويل</strong> من المكتب</p>
          <p style={{ fontSize: 12 }}>4️⃣ أدخل المبلغ + TxID + الصورة هنا واضغط تأكيد</p>
        </div>
        <hr style={{ borderColor: 'var(--border)', margin: '15px 0' }} />
        <div style={{ background: 'var(--bg3)', padding: 12, borderRadius: 8, marginBottom: 12, fontSize: 12 }}><strong>نطاقات الخطط:</strong> Bronze $10-49 ($4/week) | Silver $50-99 ($8/week) | Platinum $100-499 ($20/week) | Gold $500+ ($100/week)</div>
        <input style={{ ...s.inp, borderColor: Number(depAmount) >= 10 ? 'var(--accent)' : 'var(--danger)' }} type="number" placeholder="المبلغ (الحد الأدنى $10)" value={depAmount} onChange={e => setDepAmount(e.target.value)} min="10" />
        <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 8 }}>{Number(depAmount) >= 10 ? 'الخطة: ' + (Number(depAmount) >= 500 ? 'Gold - $100/week' : Number(depAmount) >= 100 ? 'Platinum - $20/week' : Number(depAmount) >= 50 ? 'Silver - $8/week' : 'Bronze - $4/week') : 'أدخل المبلغ لعرض الخانة'}</div>
        <input style={s.inp} placeholder="TxID — رقم المعاملة من صاحب المكتب" value={depTx} onChange={e => setDepTx(e.target.value)} />
        <div style={{ margin: '12px 0' }}>
          <label style={{ fontSize: 13, color: 'var(--warning)', display: 'block', marginBottom: 6 }}>📷 صورة إثبات الدفع (إجباري):</label>
          <input type="file" accept="image/*" onChange={handleProofImage} style={{ ...s.inp, padding: 8 }} />
          {proofPreview && <div style={{ marginTop: 8 }}><img src={proofPreview} alt="preview" style={{ maxWidth: 200, maxHeight: 200, borderRadius: 8, border: '1px solid var(--border)' }} /><p style={{ fontSize: 11, color: 'var(--accent)', marginTop: 4 }}>✅ تم اختيار الصورة</p></div>}
        </div>
        <button style={{ ...s.btn('#28a745'), opacity: depLoading ? 0.7 : 1 }} onClick={doDeposit} disabled={depLoading}>{depLoading ? 'جاري الإرسال...' : 'تأكيد الإيداع'}</button>
      </div>)}
...
function Admin({ onLogout }) {
  ...
  const loadUsers = async () => { try { const r = await API.get('/admin/users'); if (r.data.success) setUsers(r.data.users || []); } catch(e) { setUsers([]); } };
  const loadTxns = async () => { try { const r = await API.get('/admin/transactions'); if (r.data.success) setTxns(r.data.transactions || []); } catch(e) { setTxns([]); } };
