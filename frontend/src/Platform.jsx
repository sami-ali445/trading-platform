...
function Admin({ onLogout }) {
  ...
  return (
    <div style={s.dash}>
      ...
      {tab === 'users' && <div style={s.tc}><h3>👥 المستخدمين ({users.length})</h3><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>المستخدم</th><th>الرصيد</th><th>الخطة</th><th>العمولات</th><th>الإحالات</th></tr></thead><tbody>{users.map(u => <tr key={u.username}><td>{u.username}</td><td>${u.balance?.toFixed(2)}</td><td>{u.activePlan || '—'}</td><td style={{ color: 'var(--accent)' }}>${(u.totalCommission || 0).toFixed(2)}</td><td>{u.activeReferrals || 0}/{u.totalReferrals || 0}</td></tr>)}</tbody></table></div></div>}
